...
public:
    ...
    static void TryBroadcastMessage( const UObject * WorldContextObject, FGameplayTag Channel, const FMessageStructType & Message )
    {
        if ( auto * router = Get( WorldContextObject ) )
        {
            router->BroadcastMessage( Channel, Message );
        }
    }
