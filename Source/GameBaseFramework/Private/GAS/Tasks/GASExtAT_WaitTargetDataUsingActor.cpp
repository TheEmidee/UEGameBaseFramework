#include "GAS/Tasks/GASExtAT_WaitTargetDataUsingActor.h"
...
void UGASExtAT_WaitTargetDataUsingActor::InitializeTargetActor() const
{
    ...
    TargetActor->CanceledDelegate.AddUObject( const_cast< UGASExtAT_WaitTargetDataUsingActor * >( this ), &UGASExtAT_WaitTargetDataUsingActor::OnTargetDataCancelledCallback );
}

void UGASExtAT_WaitTargetDataUsingActor::FinalizeTargetActor() const
{
    check( TargetActor != nullptr );
    check( Ability != nullptr );
...
void UGASExtAT_WaitTargetDataUsingActor::OnDestroy( const bool ability_ended )
{
    if ( TargetActor != nullptr )
    {
        if ( auto * trace_target_actor = Cast< AGASExtGameplayAbilityTargetActor >( TargetActor ) )
        {
...
bool UGASExtAT_WaitTargetDataUsingActor::ShouldReplicateDataToServer() const
{
    if ( Ability == nullptr || TargetActor == nullptr )
    {
        return false;
