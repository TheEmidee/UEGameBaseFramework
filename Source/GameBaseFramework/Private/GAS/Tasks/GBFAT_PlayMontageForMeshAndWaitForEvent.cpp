#include "GAS/Tasks/GBFAT_PlayMontageForMeshAndWaitForEvent.h"
...
UGBFAT_PlayMontageForMeshAndWaitForEvent::UGBFAT_PlayMontageForMeshAndWaitForEvent()
{
    Rate = 1.0f;
    StopsWhenAbilityEnds = true;
}

void UGBFAT_PlayMontageForMeshAndWaitForEvent::Activate()
{
    if ( Ability == nullptr )
    {
        return;
    ...
    if ( auto * asc = GetTargetASC() )
    {
        if ( auto * anim_instance = Mesh->GetAnimInstance() )
        {
            ...
            EventHandle = asc->AddGameplayEventTagContainerDelegate( EventTags, FGameplayEventTagMulticastDelegate::FDelegate::CreateUObject( this, &UGBFAT_PlayMontageForMeshAndWaitForEvent::OnGameplayEvent ) );
            ...
            if ( asc->PlayMontageForMesh( Ability, Mesh, MontageToPlay, Rate, StartSection, MustReplicateMontage ) > 0.f )
            {
                ...
                CancelledHandle = Ability->OnGameplayAbilityCancelled.AddUObject( this, &UGBFAT_PlayMontageForMeshAndWaitForEvent::OnAbilityCancelled );

                BlendingOutDelegate.BindUObject( this, &UGBFAT_PlayMontageForMeshAndWaitForEvent::OnMontageBlendingOut );
                anim_instance->Montage_SetBlendingOutDelegate( BlendingOutDelegate, MontageToPlay );

                MontageEndedDelegate.BindUObject( this, &UGBFAT_PlayMontageForMeshAndWaitForEvent::OnMontageEnded );
        ...
        {
            UE_LOG( LogTemp, Warning, TEXT( "UGBFAT_PlayMontageForMeshAndWaitForEvent call to PlayMontage failed!" ) );
        }
    }
    else
    {
        UE_LOG( LogTemp, Warning, TEXT( "UGBFAT_PlayMontageForMeshAndWaitForEvent called on invalid AbilitySystemComponent" ) );
    }

    if ( !montage_was_played )
    {
        UE_LOG( LogTemp, Warning, TEXT( "UGBFAT_PlayMontageForMeshAndWaitForEvent called in Ability %s failed to play montage %s; Task Instance Name %s." ), *Ability->GetName(), *GetNameSafe( MontageToPlay ), *InstanceName.ToString() );
...
void UGBFAT_PlayMontageForMeshAndWaitForEvent::ExternalCancel()
{
...
FString UGBFAT_PlayMontageForMeshAndWaitForEvent::GetDebugString() const
{
...
void UGBFAT_PlayMontageForMeshAndWaitForEvent::OnDestroy( const bool ability_ended )
{
...
UGBFAT_PlayMontageForMeshAndWaitForEvent * UGBFAT_PlayMontageForMeshAndWaitForEvent::PlayMontageForMeshAndWaitForEvent(
    ...
    const float override_blend_out_time_for_stop_when_end_ability )
{
    ...
    auto * my_obj = NewAbilityTask< UGBFAT_PlayMontageForMeshAndWaitForEvent >( owning_ability, task_instance_name );
    my_obj->Mesh = mesh;
...
bool UGBFAT_PlayMontageForMeshAndWaitForEvent::StopPlayingMontage( const float override_blend_out_time ) const
{
    if ( Mesh == nullptr )
    {
        return false;
...
UGBFAbilitySystemComponent * UGBFAT_PlayMontageForMeshAndWaitForEvent::GetTargetASC() const
{
    return Cast< UGBFAbilitySystemComponent >( AbilitySystemComponent );
}

void UGBFAT_PlayMontageForMeshAndWaitForEvent::OnMontageBlendingOut( UAnimMontage * montage, const bool was_interrupted ) const
{
...
void UGBFAT_PlayMontageForMeshAndWaitForEvent::OnAbilityCancelled() const
{
...
void UGBFAT_PlayMontageForMeshAndWaitForEvent::OnMontageEnded( UAnimMontage * montage, const bool was_interrupted )
{
    if ( !was_interrupted )
    {
        if ( ShouldBroadcastAbilityTaskDelegates() )
...
void UGBFAT_PlayMontageForMeshAndWaitForEvent::OnGameplayEvent( const FGameplayTag event_tag, const FGameplayEventData * payload ) const
{
    if ( ShouldBroadcastAbilityTaskDelegates() )
    {
