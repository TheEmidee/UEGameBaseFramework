...
bool FGBFCameraModifierUtilsLibrary::SphereTraceSingleByChannel(AActor * view_target, FHitResult & hit_result, const FVector & start, const FVector & end, float radius, const TEnumAsByte<ECollisionChannel> collision_channel, bool draw_debug)
{
    const auto draw_debug_type = draw_debug ? EDrawDebugTrace::ForOneFrame : EDrawDebugTrace::None;
    const auto object_type_query = UCollisionProfile::Get()->ConvertToTraceType( collision_channel );
    return UKismetSystemLibrary::SphereTraceSingle( view_target, start, end, radius, object_type_query, false, { view_target }, draw_debug_type, hit_result, false );
}