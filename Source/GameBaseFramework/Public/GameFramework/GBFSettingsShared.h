...
#include "GBFSettingsShared.generated.h"
...
public:
    ...
    static UGBFSettingsShared * CreateTemporarySettings( const UGBFLocalPlayer * local_player );

    /** Synchronously loads a settings object, this is not valid to call before login */
    static UGBFSettingsShared * LoadOrCreateSettings( const UGBFLocalPlayer * local_player );

    DECLARE_DELEGATE_OneParam( FGBFOnSettingsLoadedEvent, UGBFSettingsShared * Settings );

    /** Starts an async load of the settings object, calls Delegate on completion */
    static bool AsyncLoadOrCreateSettings( const UGBFLocalPlayer * local_player, const TSubclassOf< UGBFSettingsShared > & save_game_class, FGBFOnSettingsLoadedEvent delegate );
